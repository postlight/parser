...
var _extends$1 = _interopDefault$1(_extends);
...
var _defineProperty$1 = _interopDefault$1(_defineProperty);
...
function getEncoding$1(str) {
  if (ENCODING_RE$1.test(str)) {
    return ENCODING_RE$1.exec(str)[1];
  }
  ...
  return new _Promise(function (resolve, reject) {
    request$1(options, function (err, response, body) {
      if (err) {
        ...
        var encoding = getEncoding$1(response.headers['content-type']);
        ...
        if (typeof body !== 'string') {
          var $ = cheerio$1.load(iconv.decode(body, 'utf8'));
          var contentType = $('meta[http-equiv=content-type]').attr('content');
          var properEncoding = getEncoding$1(contentType);
...
function convertMetaProp($, from$$1, to) {
  $('meta[' + from$$1 + ']').each(function (_, node) {
    var $node = $(node);
...
// Spacer images to be removed
var SPACER_RE$1 = new RegExp('transparent|spacer|blank', 'i');
...
// and then tested for existence using re:test, so may contain simple,
// non-pipe style regular expression queries if necessary.
var UNLIKELY_CANDIDATES_BLACKLIST$2 = ['ad-break', 'adbox', 'advert', 'addthis', 'agegate', 'aux', 'blogger-labels', 'combx', 'comment', 'conversation', 'disqus', 'entry-unrelated', 'extra', 'foot',
// 'form', // This is too generic, has too many false positives
'header', 'hidden', 'loader', 'login', // Note: This can hit 'blogindex'.
'menu', 'meta', 'nav', 'outbrain', 'pager', 'pagination', 'predicta', // readwriteweb inline ad box
'presence_control_external', // lifehacker.com container full of false positives
...
// Match any phrase that looks like it could be page, or paging, or pagination
var PAGE_RE$1 = new RegExp('pag(e|ing|inat)', 'i');
...
var BLOCK_LEVEL_TAGS_RE$2 = new RegExp('^(' + BLOCK_LEVEL_TAGS$2.join('|') + ')$', 'i');
...
function stripUnlikelyCandidates$1($) {
  ...
  $('*').not('a').each(function (index, node) {
    ...
    if (CANDIDATES_WHITELIST$2.test(classAndId)) {
      return;
    } else if (CANDIDATES_BLACKLIST$2.test(classAndId)) {
      $node.remove();
    }
  });
...
function markToKeep$1(article, $, url) {
  ...
  if (url) {
    ...
    tags = [].concat(_toConsumableArray$1(tags), ['iframe[src^="' + protocol + '//' + hostname + '"]']);
  }
  ...
  $article.find('*').each(function (index, node) {
    ...
    setAttrs$1(node, _Reflect$ownKeys$1(attrs).reduce(function (acc, attr) {
      if (WHITELIST_ATTRS_RE$1.test(attr)) {
        return _extends$1({}, acc, _defineProperty$1({}, attr, attrs[attr]));
      }
...
// The above list, joined into a matching regular expression
var POSITIVE_SCORE_RE$1$1 = new RegExp(POSITIVE_SCORE_HINTS$1$1.join('|'), 'i');
...
'scroll', 'secondary', 'share', 'shopping', 'shoutbox', 'side', 'sidebar', 'sponsor', 'stamp', 'sub', 'summary', 'tags', 'tools', 'widget'];
// The above list, joined into a matching regular expression
var NEGATIVE_SCORE_RE$1$1 = new RegExp(NEGATIVE_SCORE_HINTS$1$1.join('|'), 'i');
...
function absolutize$1($, rootUrl, attr, $content) {
  $('[' + attr + ']', $content).each(function (_, node) {
    ...
    if (url) {
      var absoluteUrl = URL$1.resolve(rootUrl, url);
...
var CustomExtractors = _Object$freeze({
  ...
  extract: function extract(_ref) {
    ...
    try {
      for (var _iterator = _getIterator$1(BYLINE_SELECTORS_RE), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        ...
        var _ref3 = _slicedToArray$1(_ref4, 2);
...
function scoreSimilarity(score, articleUrl, href) {
  ...
  // similarity.
  if (score > 0) {
    var similarity = new difflib$1.SequenceMatcher(null, articleUrl, href).ratio();
...
function select(opts) {
  ...
  if (defaultCleaner) {
    return Cleaners[type](result, _extends$1({}, opts, extractionOpts));
  }
